...
const G = `
  ...
  *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
  ...
  html { font-size:16px; }
  body { font-family:'Hind Siliguri','Sora',sans-serif; background:var(--bg); color:var(--text); min-height:100vh; min-height:100dvh; overflow-x:hidden; width:100%; }
  ...
  .btn { border:none; cursor:pointer; font-family:'Hind Siliguri',sans-serif; font-weight:600; transition:all 0.25s; display:inline-flex; align-items:center; gap:8px; justify-content:center; -webkit-tap-highlight-color:transparent; }
  .btn-grad { background:linear-gradient(135deg,#7c6af7,#a78bfa); color:#fff; border-radius:12px; font-size:15px; box-shadow:0 4px 20px rgba(124,106,247,0.35); }
  ...
  .btn-del { background:rgba(255,107,107,0.1); color:#ff6b6b; padding:6px 12px; border-radius:8px; font-size:12px; border:1px solid rgba(255,107,107,0.2); }
  .btn-del:hover { background:rgba(255,107,107,0.22); }

  .nav-btn { display:flex; align-items:center; gap:12px; padding:11px 14px; border-radius:12px; border:none; width:100%; font-family:'Hind Siliguri',sans-serif; font-size:14px; font-weight:500; cursor:pointer; transition:all 0.2s; text-align:left; color:var(--text3); background:transparent; -webkit-tap-highlight-color:transparent; }
...
const AuthPage = ({ onLogin }) => {
  ...
  const submit = async () => {
    setErr(""); setOk("");
    if (mode === "signup" && !name.trim()) { setErr("আপনার নাম দিন"); return; }
    ...
    setLoading(true);
    const r = mode === "login"
      ? await MockAuth.signIn(email, pass)
      : await MockAuth.signUp(email, pass, name.trim());
    setLoading(false);
    if (r.error) { setErr(r.error.message); return; }
    if (mode === "signup") {
      setOk("✅ অ্যাকাউন্ট তৈরি হয়েছে! Email verify করুন তারপর লগইন করুন।");
      setMode("login"); setPass(""); setName(""); return;
    }
    onLogin(r.data.user);
  };

  const lbl = (text) => (
    <label style={{ display:"block", fontSize:11, fontWeight:700, color:"var(--text3)", marginBottom:8, letterSpacing:"1px", textTransform:"uppercase" }}>{text}</label>
  );

  return (
    <div className="auth-screen">
      <div style={{ position:"fixed", inset:0, zIndex:0, pointerEvents:"none" }}>
        <div style={{ position:"absolute", top:"8%", left:"10%", width:500, height:500, background:"radial-gradient(circle,rgba(124,106,247,0.13) 0%,transparent 65%)", borderRadius:"50%" }} />
        <div style={{ position:"absolute", bottom:"8%", right:"8%", width:380, height:380, background:"radial-gradient(circle,rgba(0,212,170,0.08) 0%,transparent 65%)", borderRadius:"50%" }} />
      </div>

      <div className="auth-card fade-up">
        <div style={{ textAlign:"center", marginBottom:32 }}>
          <div style={{ width:70, height:70, borderRadius:22, background:"linear-gradient(135deg,rgba(124,106,247,0.25),rgba(167,139,250,0.08))", border:"1px solid rgba(124,106,247,0.3)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:32, margin:"0 auto 16px", animation:"float 3s ease-in-out infinite" }}>💰</div>
          <h1 style={{ fontFamily:"'Sora',sans-serif", fontSize:26, fontWeight:800, color:"var(--text)", marginBottom:6, letterSpacing:"-0.5px" }}>অর্থ ব্যবস্থাপক</h1>
          <p style={{ color:"var(--text3)", fontSize:14 }}>আপনার স্মার্ট আর্থিক সহকারী</p>
        </div>

        <div style={{ display:"flex", background:"rgba(0,0,0,0.35)", borderRadius:13, padding:4, marginBottom:26, border:"1px solid var(--border)" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          {mode === "signup" && (
            <div>
              ...
              <div style={{ position:"relative" }}>
                ...
                <input className="inp" type="text" placeholder="আপনার পুরো নাম" value={name} onChange={e=>setName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&submit()} style={{ paddingLeft:44 }} />
              </div>
            </div>
          )}
          <div>
          ...
          {err && (
            <div style={{ background:"rgba(255,107,107,0.1)", border:"1px solid rgba(255,107,107,0.25)", borderRadius:10, padding:"11px 14px", color:"#ff6b6b", fontSize:13, display:"flex", gap:8, alignItems:"flex-start" }}>
              <span style={{ flexShrink:0 }}>⚠️</span><span>{err}</span>
            </div>
          )}
          {ok && (
            <div style={{ background:"rgba(0,212,170,0.1)", border:"1px solid rgba(0,212,170,0.25)", borderRadius:10, padding:"11px 14px", color:"#00d4aa", fontSize:13 }}>{ok}</div>
          )}
        ...
        <p style={{ marginTop:22, textAlign:"center", fontSize:12, color:"var(--text3)", display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
...
const StatCard = ({ icon, label, value, color, sub, delay=0 }) => (
  <div className="card fade-up" style={{ padding:18, animationDelay:`${delay}ms`, position:"relative", overflow:"hidden" }}>
    <div style={{ position:"absolute", top:-15, right:-15, width:80, height:80, background:`radial-gradient(circle,${color}15 0%,transparent 70%)`, borderRadius:"50%", pointerEvents:"none" }} />
    <div style={{ width:40, height:40, borderRadius:11, background:`${color}18`, border:`1px solid ${color}28`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, marginBottom:12 }}>{icon}</div>
    <div className="stat-num" style={{ fontFamily:"'Sora',sans-serif", fontSize:20, fontWeight:700, color:color, marginBottom:3 }}>৳{Number(value).toLocaleString()}</div>
  ...
  return (
    <div className="card fade-up" style={{ padding:24 }}>
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:20 }}>
        <div style={{ width:40, height:40, borderRadius:11, background:"rgba(124,106,247,0.15)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, flexShrink:0 }}>➕</div>
        <div>
          <h3 style={{ fontSize:15, fontWeight:700 }}>নতুন খরচ যোগ করুন</h3>
      ...
      <div className="form-cols" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
        <div style={{ gridColumn:"1/-1" }}>
          <label style={{ display:"block", fontSize:11, fontWeight:700, color:"var(--text3)", marginBottom:7, letterSpacing:"1px", textTransform:"uppercase" }}>খরচের বিবরণ</label>
          <input className="inp" placeholder="কিসের জন্য খরচ হলো?" value={desc} onChange={e=>setDesc(e.target.value)} />
        </div>
        <div>
          <label style={{ display:"block", fontSize:11, fontWeight:700, color:"var(--text3)", marginBottom:7, letterSpacing:"1px", textTransform:"uppercase" }}>পরিমাণ (৳)</label>
          <input className="inp" type="number" placeholder="০.০০" value={amount} onChange={e=>setAmount(e.target.value)} min="0" />
        </div>
        <div>
          <label style={{ display:"block", fontSize:11, fontWeight:700, color:"var(--text3)", marginBottom:7, letterSpacing:"1px", textTransform:"uppercase" }}>তারিখ</label>
          <input className="inp" type="date" value={date} onChange={e=>setDate(e.target.value)} />
        </div>
        <div style={{ gridColumn:"1/-1" }}>
          <label style={{ display:"block", fontSize:11, fontWeight:700, color:"var(--text3)", marginBottom:9, letterSpacing:"1px", textTransform:"uppercase" }}>বিভাগ</label>
          <div style={{ display:"flex", flexWrap:"wrap", gap:7 }}>
            {CATEGORIES.map(c => (
              <button key={c.id} onClick={() => setCat(c.id)} className="btn" style={{ padding:"6px 12px", borderRadius:20, fontSize:13, background:cat===c.id?c.bg:"rgba(255,255,255,0.04)", color:cat===c.id?c.color:"var(--text3)", border:cat===c.id?`1px solid ${c.color}50`:"1px solid var(--border)", fontWeight:cat===c.id?600:400, transform:cat===c.id?"scale(1.05)":"scale(1)" }}>
                {c.icon} {c.label}
              </button>
            ))}
          </div>
...
const ExpenseList = ({ expenses, onDelete }) => {
  ...
  return (
    <div className="card" style={{ overflow:"hidden" }}>
      <div style={{ padding:"15px 20px 12px", borderBottom:"1px solid var(--border)", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          <div style={{ width:8, height:8, borderRadius:"50%", background:"var(--accent)", boxShadow:"0 0 8px var(--accent)" }} />
          <span style={{ fontSize:14, fontWeight:600 }}>খরচের তালিকা</span>
        </div>
      ...
      <div style={{ maxHeight:420, overflowY:"auto" }}>
        {expenses.map((exp) => {
          ...
          return (
            <div key={exp.id} style={{ padding:"12px 20px", display:"flex", alignItems:"center", gap:12, borderBottom:"1px solid var(--border)", transition:"background 0.2s" }}
              onMouseEnter={e=>e.currentTarget.style.background="rgba(255,255,255,0.02)"}
              onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
              <div style={{ width:38, height:38, borderRadius:10, background:c.bg, display:"flex", alignItems:"center", justifyContent:"center", fontSize:17, flexShrink:0 }}>{c.icon}</div>
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontSize:13, fontWeight:500, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis", marginBottom:3 }}>{exp.description}</div>
                <div style={{ fontSize:11, color:"var(--text3)", display:"flex", gap:6, flexWrap:"wrap", alignItems:"center" }}>
                  <span style={{ background:c.bg, color:c.color, padding:"1px 7px", borderRadius:10 }}>{c.icon} {exp.category}</span>
                  <span>{new Date(exp.date).toLocaleDateString("bn-BD")}</span>
                </div>
              </div>
              <div style={{ fontSize:14, fontWeight:700, color:c.color, fontFamily:"'Sora',sans-serif", flexShrink:0 }}>৳{Number(exp.amount).toLocaleString()}</div>
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
      <div style={{ display:"flex", gap:8 }}>
        {[["weekly","সাপ্তাহিক"],["monthly","মাসিক"]].map(([v,l])=>(
          <button key={v} className="btn" onClick={()=>setPeriod(v)} style={{ padding:"9px 18px", borderRadius:10, fontSize:14, background:period===v?"linear-gradient(135deg,#7c6af7,#a78bfa)":"rgba(255,255,255,0.05)", color:period===v?"#fff":"var(--text3)", border:"1px solid var(--border)", fontWeight:period===v?600:400 }}>{l}</button>
        ))}
      </div>
      <div className="card" style={{ padding:22 }}>
        <h3 style={{ fontSize:15, fontWeight:700, marginBottom:20, display:"flex", alignItems:"center", gap:8 }}>📊 বিভাগ অনুযায়ী খরচ</h3>
        {sorted.length===0 ? (
            ...
            <div key={cat} style={{ marginBottom:15 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  <span style={{ width:26, height:26, borderRadius:7, background:c.bg, display:"inline-flex", alignItems:"center", justifyContent:"center", fontSize:13 }}>{c.icon}</span>
                  <span style={{ fontSize:13, color:"var(--text2)" }}>{cat}</span>
                </div>
                <div style={{ display:"flex", gap:10, alignItems:"center" }}>
                  <span style={{ fontSize:11, color:"var(--text3)" }}>{pct.toFixed(1)}%</span>
                  <span style={{ fontSize:13, fontWeight:700, color:c.color, fontFamily:"'Sora',sans-serif" }}>৳{amt.toLocaleString()}</span>
                </div>
              </div>
        ...
        {total>0&&(
          <div style={{ marginTop:16, paddingTop:16, borderTop:"1px solid var(--border)", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            ...
            <span style={{ fontFamily:"'Sora',sans-serif", fontSize:20, fontWeight:700, color:"var(--accent2)" }}>৳{total.toLocaleString()}</span>
          </div>
        )}
      </div>
    </div>
    ...
    <div className="card" style={{ padding:24 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20, flexWrap:"wrap", gap:12 }}>
        <div style={{ display:"flex", gap:12, alignItems:"center" }}>
          <div style={{ width:46, height:46, borderRadius:13, background:"linear-gradient(135deg,rgba(124,106,247,0.2),rgba(167,139,250,0.05))", border:"1px solid rgba(124,106,247,0.25)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:22, flexShrink:0 }}>🤖</div>
          <div>
            <h3 style={{ fontSize:15, fontWeight:700 }}>AI সঞ্চয় উপদেষ্টা</h3>
            <p style={{ fontSize:12, color:"var(--text3)", marginTop:2 }}>Claude AI বিশ্লেষণ করবে</p>
          </div>
        </div>
        <button className="btn btn-grad" onClick={fetchAdvice} disabled={loading} style={{ padding:"10px 18px", fontSize:13, borderRadius:10 }}>
          {loading?<><div className="spin"/><span>বিশ্লেষণ...</span></>:done?"🔄 আবার":"💡 পরামর্শ নিন"}
        </button>
      </div>
      {!done&&!loading&&(
        <div style={{ textAlign:"center", padding:"36px 20px", border:"1px dashed rgba(255,255,255,0.07)", borderRadius:14 }}>
          <div style={{ fontSize:42, marginBottom:12, animation:"float 3s ease-in-out infinite" }}>🧠</div>
          <p style={{ color:"var(--text2)", fontSize:14, marginBottom:5 }}>AI পরামর্শ নিতে প্রস্তুত?</p>
          <p style={{ color:"var(--text3)", fontSize:12 }}>উপরের বোতামে ক্লিক করুন</p>
        </div>
      )}
      {loading&&(
        <div style={{ textAlign:"center", padding:"36px 20px" }}>
          <div style={{ fontSize:34, marginBottom:12, animation:"pulse 1.5s ease-in-out infinite" }}>⚡</div>
      ...
      {done&&!loading&&(
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          {advice.split("\n").filter(l=>l.trim()).map((line,i)=>(
            <div key={i} className="fade-up" style={{ padding:"13px 16px", background:"rgba(124,106,247,0.06)", border:"1px solid rgba(124,106,247,0.12)", borderLeft:"3px solid var(--accent)", borderRadius:11, animationDelay:`${i*120}ms` }}>
...
const Dashboard = ({ user, onLogout }) => {
  ...
  const goTab = (id) => { setTab(id); if (window.innerWidth < 768) setSideOpen(false); };
    ...
    <div className="app-layout">
      ...
      <div className={`sidebar ${sideOpen?"open":""}`}>
        <div style={{ padding:"4px 6px 18px", borderBottom:"1px solid var(--border)", marginBottom:12 }}>
          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:6 }}>
            ...
            <span style={{ fontFamily:"'Sora',sans-serif", fontSize:13, fontWeight:700 }}>অর্থ ব্যবস্থাপক</span>
          </div>
          <div style={{ fontSize:13, fontWeight:600, color:"var(--text2)", marginBottom:2 }}>👋 {displayName}</div>
          <div style={{ fontSize:11, color:"var(--text3)", wordBreak:"break-all" }}>{user.email}</div>
        </div>
        <nav style={{ flex:1, display:"flex", flexDirection:"column", gap:3 }}>
          {navs.map(n=>(
            <button key={n.id} className={`nav-btn ${tab===n.id?"on":""}`} onClick={()=>goTab(n.id)}>
              <div style={{ width:32, height:32, borderRadius:9, background:tab===n.id?"rgba(124,106,247,0.2)":"rgba(255,255,255,0.04)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:15, flexShrink:0, transition:"background 0.2s" }}>{n.icon}</div>
              <span style={{ flex:1 }}>{n.label}</span>
              {tab===n.id&&<div style={{ width:6, height:6, borderRadius:"50%", background:"var(--accent)", boxShadow:"0 0 8px var(--accent)" }}/>}
            </button>
          ))}
        </nav>
        <button className="btn" onClick={onLogout} style={{ width:"100%", padding:"11px", borderRadius:11, background:"rgba(255,107,107,0.08)", color:"#ff6b6b", border:"1px solid rgba(255,107,107,0.15)", fontSize:14, marginTop:8 }}>
          🚪 লগআউট
        </button>
      </div>
      ...
      <div className="main-area">
        <div className="topbar">
          <button onClick={()=>setSideOpen(!sideOpen)} className="btn" style={{ width:36, height:36, borderRadius:9, background:"rgba(255,255,255,0.05)", color:"var(--text2)", border:"1px solid var(--border)", fontSize:15, flexShrink:0 }}>☰</button>
          <div style={{ flex:1, minWidth:0 }}>
            <h2 style={{ fontSize:14, fontWeight:700, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{navs.find(n=>n.id===tab)?.icon} {navs.find(n=>n.id===tab)?.label}</h2>
            <p className="hide-mob" style={{ fontSize:11, color:"var(--text3)" }}>{new Date().toLocaleDateString("bn-BD",{weekday:"long",year:"numeric",month:"long",day:"numeric"})}</p>
          </div>
          <div style={{ width:34, height:34, borderRadius:9, background:"linear-gradient(135deg,rgba(124,106,247,0.25),rgba(167,139,250,0.1))", display:"flex", alignItems:"center", justifyContent:"center", fontSize:15, flexShrink:0, fontWeight:700, color:"var(--accent2)" }}>
        ...
        <div className="page">
          ...
          {tab==="expenses" && (
            <div style={{ maxWidth:620, width:"100%" }}>
              ...
              <div style={{ marginTop:18 }}><ExpenseList expenses={expenses} onDelete={del}/></div>
            </div>
          )}
          {tab==="analytics" && <div style={{ maxWidth:680, width:"100%" }}><Analytics expenses={expenses}/></div>}
          {tab==="ai" && <div style={{ maxWidth:680, width:"100%" }}><AIAdvisor expenses={expenses} userEmail={user.email}/></div>}
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  if (checking) return (
    <div style={{ width:"100vw", height:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"var(--bg)", gap:20 }}>
      <div style={{ fontSize:38, animation:"float 2s ease-in-out infinite" }}>💰</div>
      <div className="spin" style={{ width:30, height:30, borderWidth:3, borderColor:"rgba(124,106,247,0.2)", borderTopColor:"#7c6af7" }}/>
    </div>
  );
