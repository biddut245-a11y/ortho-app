...
// ============================================================
// SUPABASE CONFIG — এখানে আপনার keys বসান
// ============================================================
...
৩-৪টি নির্দিষ্ট ও ব্যবহারিক পরামর্শ দাও। প্রতিটি পরামর্শ আলাদা লাইনে দাও।`;
  ...
  try {
    ...
    return "AI পরামর্শ এই মুহূর্তে উপলব্ধ নেই।";
  }
};
...
const globalStyles = `
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  body { font-family: 'Hind Siliguri', sans-serif; background: #0a0f1e; color: #e2e8f0; min-height: 100vh; }
  ...
  .glass { background: rgba(15,23,42,0.7); backdrop-filter: blur(20px); border: 1px solid rgba(99,102,241,0.15); border-radius: 16px; }
  .glass-light { background: rgba(30,41,59,0.5); backdrop-filter: blur(10px); border: 1px solid rgba(148,163,184,0.1); border-radius: 12px; }
  .btn-primary { background: linear-gradient(135deg,#6366f1,#8b5cf6); color: white; border: none; padding: 12px 24px; border-radius: 10px; font-family: 'Hind Siliguri',sans-serif; font-size: 15px; font-weight: 600; cursor: pointer; transition: all 0.3s; display: inline-flex; align-items: center; gap: 8px; }
  .btn-primary:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(99,102,241,0.4); }
  .btn-primary:disabled { opacity: 0.6; cursor: not-allowed; transform: none; }
  .btn-danger { background: rgba(239,68,68,0.15); color: #f87171; border: 1px solid rgba(239,68,68,0.3); padding: 6px 12px; border-radius: 8px; font-family: 'Hind Siliguri',sans-serif; font-size: 13px; cursor: pointer; transition: all 0.2s; }
  .btn-danger:hover { background: rgba(239,68,68,0.25); }
  .input-field { width: 100%; background: rgba(15,23,42,0.8); border: 1px solid rgba(99,102,241,0.2); border-radius: 10px; padding: 12px 16px; color: #e2e8f0; font-family: 'Hind Siliguri',sans-serif; font-size: 15px; transition: border-color 0.2s,box-shadow 0.2s; outline: none; }
  ...
  .tab-btn { padding: 8px 20px; border-radius: 8px; border: none; font-family: 'Hind Siliguri',sans-serif; font-size: 14px; font-weight: 500; cursor: pointer; transition: all 0.2s; }
  ...
  .nav-item { display: flex; align-items: center; gap: 10px; padding: 10px 16px; border-radius: 10px; border: none; width: 100%; font-family: 'Hind Siliguri',sans-serif; font-size: 14px; font-weight: 500; cursor: pointer; transition: all 0.2s; text-align: left; }
...
const AuthPage = ({ onLogin }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (mode === "signup") {
      setSuccess("অ্যাকাউন্ট তৈরি হয়েছে! লগইন করুন।");
      setMode("login");
      setPassword("");
      return;
    }
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px", position: "relative", zIndex: 1 }}>
      <div className="glass fade-in" style={{ width: "100%", maxWidth: "440px", padding: "48px 40px" }}>
        <div style={{ textAlign: "center", marginBottom: "36px" }}>
          ...
          <h1 style={{ fontSize: "26px", fontWeight: "700", background: "linear-gradient(135deg,#a5b4fc,#c4b5fd)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", marginBottom: "6px" }}>অর্থ ব্যবস্থাপক</h1>
        ...
        <p style={{ textAlign: "center", fontSize: "12px", color: "#334155", marginTop: "24px" }}>🔒 আপনার তথ্য সম্পূর্ণ নিরাপদ ও ব্যক্তিগত</p>
      </div>
    </div>
  );
};
...
const ExpenseForm = ({ userId, onAdd }) => {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    await MockDB.addExpense({ user_id: userId, description: desc, amount: parseFloat(amount), category, date });
  ...
  return (
    <div className="glass" style={{ padding: "28px" }}>
      <h3 style={{ fontSize: "17px", fontWeight: "600", color: "#c7d2fe", marginBottom: "20px" }}>➕ নতুন খরচ যোগ করুন</h3>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
        ...
        <div style={{ gridColumn: "1/-1" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
            {CATEGORIES.map((cat) => (
              <button key={cat.id} onClick={() => setCategory(cat.id)} style={{ padding: "6px 14px", borderRadius: "20px", border: "none", cursor: "pointer", fontSize: "13px", fontFamily: "inherit", transition: "all 0.2s", background: category === cat.id ? cat.color + "33" : "rgba(30,41,59,0.5)", color: category === cat.id ? cat.color : "#64748b", outline: category === cat.id ? `1px solid ${cat.color}60` : "1px solid transparent" }}>
                {cat.icon} {cat.label}
              </button>
            ))}
          </div>
...
const StatCard = ({ icon, label, value, color, sub }) => (
  <div className="glass-light fade-in" style={{ padding: "20px", position: "relative", overflow: "hidden" }}>
    ...
    <div style={{ fontSize: "22px", fontWeight: "700", color: color, marginBottom: "4px" }}>৳{Number(value).toLocaleString()}</div>
    ...
    <div className="glass" style={{ overflow: "hidden" }}>
      ...
      <div style={{ maxHeight: "360px", overflowY: "auto" }}>
        {expenses.map((exp, i) => {
          ...
          return (
            <div key={exp.id} className="slide-in" style={{ padding: "14px 24px", display: "flex", alignItems: "center", gap: "14px", borderBottom: "1px solid rgba(30,41,59,0.5)", animationDelay: `${i * 0.05}s` }}
              onMouseEnter={(e) => (e.currentTarget.style.background = "rgba(99,102,241,0.05)")}
              onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}>
              <div style={{ width: "40px", height: "40px", borderRadius: "10px", background: cat.color + "20", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "18px", flexShrink: 0 }}>{cat.icon}</div>
          ...
          return (
            <div key={cat} style={{ marginBottom: "14px" }}>
              ...
              <div style={{ height: "8px", background: "rgba(30,41,59,0.8)", borderRadius: "4px", overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${pct}%`, background: `linear-gradient(90deg,${catObj.color}80,${catObj.color})`, borderRadius: "4px", transition: "width 1s ease" }} />
              </div>
            </div>
          );
        })}
  ...
  return (
    <div className="glass" style={{ padding: "28px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
        <div>
          <h3 style={{ fontSize: "17px", fontWeight: "600", color: "#c7d2fe" }}>🤖 AI সঞ্চয় উপদেষ্টা</h3>
...
const Dashboard = ({ user, onLogout }) => {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", position: "relative", zIndex: 1 }}>
      <div style={{ width: sidebarOpen ? "240px" : "0", minWidth: sidebarOpen ? "240px" : "0", background: "rgba(10,15,30,0.9)", backdropFilter: "blur(20px)", borderRight: "1px solid rgba(99,102,241,0.1)", display: "flex", flexDirection: "column", padding: sidebarOpen ? "24px 16px" : "0", transition: "all 0.3s", overflow: "hidden", position: "sticky", top: 0, height: "100vh" }}>
        ...
        <div style={{ padding: "24px", flex: 1 }}>
          ...
          {activeTab === "expenses" && (
            <div style={{ maxWidth: "600px" }} className="fade-in">
              ...
              <div style={{ marginTop: "20px" }}><ExpenseList expenses={expenses} onDelete={handleDelete} /></div>
            </div>
          )}
          {activeTab === "analytics" && (
            <div style={{ maxWidth: "700px" }} className="fade-in"><Analytics expenses={expenses} /></div>
          )}
          {activeTab === "ai" && (
            <div style={{ maxWidth: "700px" }} className="fade-in"><AIAdvisor expenses={expenses} userEmail={user.email} /></div>
          )}
        </div>
      </div>
    </div>
